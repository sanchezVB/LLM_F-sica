...
| **G1.5** · corpus por um hash | 🟡 **o hash cobria 55% do corpus** | ⚠️ o peS2o (18,34 GB, 14,60 B tokens) estava FORA da tabela `ETAPAS` desde 2026-08-26, e o raiz atestava 21,79 GB de um corpus de ~40 GB. Nada acusava: o construtor só percorre a lista, e o relatório sai ✅ sobre o que ele conhece. Agora **52,40 GB · 35 etapas · 1.295 arquivos**, raiz `3113f0fe…` (com a fatia `math`+`cs` de 2026-09-12 dentro). Guarda nova: toda fonte de `filtrar_hf.FONTES` tem de ter entrada em `ETAPAS`, código contra código |
...
**A medida, em andamento** (regra no DOC-13 §9.1, aceita antes de qualquer item):
...
A regra do T1h mandou subir `gte-small` e `bge-small` para 1 M de pares; a do T1i
(`colab/t1i_pequenas_1m.py`), escrita antes, compara cada uma com o ΦEmb do sistema com IC
de **97,5%** (Bonferroni sobre 2). Os mesmos pares do T1g (T1a 1,5 M limitado a 1 M, mesma
semente), treino no Colab (2,92 h cada, 95,8 pares/s), medição local na mesma sessão:

| | pares | nDCG@10 | recall@1 | custo | − sistema [IC 97,5%] |
|---|---|---|---|---|---|
| **ΦEmb do sistema** (MiniLM-L6) | 6 M | **0,6223** | 0,4315 | 1,00× | — |
| gte-small | 1 M | 0,6014 | 0,4255 | 1,77× | **−0,0209 [−0,0323; −0,0096] · ABAIXO** |
| bge-small | 1 M | 0,6012 | 0,4255 | 1,77× | **−0,0211 [−0,0323; −0,0100] · ABAIXO** |
| GTE-base (T1g, diagnóstico) | 1 M | 0,6211 | 0,4335 | 4,16× | — |

**Desfecho pela regra: ABAIXO, as duas.** O sistema segura o lugar. No recall@1 empatam
(147 × 135, p=0,51): a perda está abaixo do topo. E ficam ~0,020 abaixo do GTE-base no
mesmo volume — a base grande ainda vale algo que a pequena não tem.

⚠️ As duas deram recall@1 e discordantes idênticos contra o sistema. Conferido que não é
troca de arquivos: pesos distintos (`d6e82ff5…` e `772e8165…`), cada um da base certa, e
nDCG e posições por item diferentes. É coincidência.

**A curva, que é o que isto ensina:**

| base | 200 mil | 1 M | ganho |
|---|---|---|---|
| MiniLM-L6 | 0,5289 | ~0,57 (interpolado entre 400 mil e 1,5 M) | — |
| gte-small | 0,5706 | 0,6014 | +0,031 |
| bge-small | 0,5692 | 0,6012 | +0,032 |
| GTE-base | 0,5964 | 0,6211 | +0,025 |

A 1 M as pequenas estão ~+0,03 acima do que o MiniLM-L6 faria no mesmo volume, e crescem
~+0,013 por dobra de pares. Para alcançar os 0,6223 do sistema precisariam de mais ~1,5
dobra (~3 M); a 6 M, extrapolando, algo perto de 0,64. **Extrapolação não é medida.** O
teste direto — uma pequena a 6 M — custaria ~17 h de T4 a 96 pares/s: não cabe numa sessão
do Kaggle (sem retomada), cabe no Colab com retomada, e compraria no máximo ~+0,02 a um
custo de serviço permanente de 1,77×.

Artefato: `data/processed/avaliacao/t1i_comparacao.json`.
...
Os quatro encoders ajustados nos mesmos 200 mil pares, medidos na mesma sessão no
protocolo do G1, com as regras escritas antes:

| @200k | recall@1 | recall@10 | MRR | nDCG@10 |
|---|---|---|---|---|
| ModernBERT-base cru (a barra) | 0,3540 | 0,7200 | 0,4774 | 0,5270 |
| CPT controle (`p_equacao` 0,0) | 0,3635 | 0,7175 | 0,4823 | 0,5297 |
| CPT tratado (`p_equacao` 0,6) | 0,3620 | 0,7290 | 0,4876 | 0,5373 |
| **GTE-base cru** | **0,4170** | **0,7880** | **0,5443** | **0,5964** |

- **Entre os braços:** +0,0076 [+0,0013; +0,0139] → **TRATADO À FRENTE**. O ganho de
  recuperação do §2.3 se repete a 150 M, **11× menor** que a 48 M (+0,084), com o IC
  encostando no zero. recall@1 empata (63 × 60, p=0,86).
- **Contra a barra:** tratado − ModernBERT cru +0,0103 [+0,0032; +0,0176] → **CPT ACIMA
  DA BARRA**.
- **A base:** GTE − ModernBERT +0,0694 → **GTE À FRENTE** (seção de 2026-09-22).

**A leitura conjunta:** 0,4 B tokens de Física com o objetivo do §2.3 compraram +0,010;
trocar de base, sem pré-treino nenhum, compra +0,069. O GTE cru fica +0,059 acima do
melhor braço do CPT. A frase da regra "o encoder do sistema passa a ser candidato a
trocar" foi escrita antes de o GTE entrar na régua — com ele dentro, o CPT sobre o
ModernBERT não é candidato a nada no produto.

⚠️ **A assimetria de spike favorece o tratado** (o controle perdeu 182 lotes em dois
rollbacks e rodou 500 passos com LR pela metade), e com um efeito de +0,0076 ela é do
tamanho que poderia explicar parte dele. Não foi medida. E a primária de MLM deu NÃO
DECIDIDO. O §2.3 chega a 150 M com uma secundária positiva marginal e uma primária nula:
**o efeito encolhe quando a base é forte**, que é o padrão de uma lacuna que a base boa
já fecha.

O que fica para o dono, no [ADR-0003 §9](docs/adr/ADR-0003-phienc-do-zero-ou-cpt.md):
o ΦEnc próprio fecha para o produto; o §2.3 fica publicável com evidência fraca e
positiva; e a pergunta do encoder do sistema segue no T1g (GTE@1M, rodando).

Artefato: `data/processed/avaliacao/t2eq_cpt_emb_comparacao.json`.

## A base do caminho B estava errada: o GTE-base supera o ModernBERT-base por +0,069 a 200 mil pares (2026-09-22)

A pergunta do braço `t2eq_emb_gte`, com a regra escrita antes (`kaggle/t2eq_emb.py` ·
REGRA_GTE): o ModernBERT-base é a base certa para o pré-treino continuado? O T1f só
tinha medido o GTE a 400 mil pares, e a barra do caminho B é a 200 mil. Uma variável,
a base — mesmos bytes de pares, lote 128, 192 tokens, semente 17. Os dois medidos na
mesma sessão, no protocolo do G1:

| @200k pares | recall@1 | recall@10 | MRR | nDCG@10 |
|---|---|---|---|---|
| ModernBERT-base (a barra) | 0,3540 | 0,7200 | 0,4774 | 0,5270 |
| **GTE-base** | **0,4170** | **0,7880** | **0,5443** | **0,5964** |

nDCG@10 gte − modernbert: **+0,0694 [+0,0580; +0,0812]**; recall@1 pareado, p < 1e-4.

**Desfecho pela regra: GTE À FRENTE.** Os dois braços do CPT, que estão sendo
ajustados agora no Colab, partiram da base errada para o produto. O que o CPT mede
sobre o §2.3 — mascarar equações inteiras ajuda ou não — continua valendo, porque
essa é uma comparação entre os dois braços. O que muda é qual encoder vai para o
sistema.

**A régua conferiu:** a barra, remedida nesta sessão, deu **0,5270** — o mesmo número
do passo 1, medido em 2026-09-17. O avaliador não derivou.

**E bate com o T1f:** o GTE-base@400k fez 0,6094, e aqui, com metade dos pares, 0,5964.
A base explica a maior parte; o volume de pares, pouco.

⚠️ A regra da secundária do CPT (`colab/t2eq_cpt_emb.py`) compara os braços com a barra
de 0,5270, e fica assim — foi escrita antes e não se mexe depois. Mas para o produto a
régua passa a ser 0,5964: para o CPT valer a troca, o melhor braço teria de ganhar
**+0,07** sobre a própria base só com 0,4 B tokens de Física.

⚠️ O GTE-base tem 110 M contra 150 M e contexto de **512 contra 8.192**. Com a
truncagem de 192 tokens do protocolo, o contexto não entra nesta medida — e a
truncagem 192 já foi medida como sem custo (2026-09-10). Entra no produto só se o
sistema passar a ler documentos longos.

Custo: 1 h 17 de T4 no Kaggle; medição local em ~10 min.
...
⚠️ **`redpajama_math_cs` é a PRIMEIRA etapa com os parâmetros capturados na
execução** — o construtor a preservou em vez de sobrescrever, e o log diz
"manifesto CAPTURADO na execução, preservado". As outras cinco seguem
`parametros_reconstruidos=true`, duas delas de propósito (ver abaixo).
