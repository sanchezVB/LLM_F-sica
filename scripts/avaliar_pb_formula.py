...
⚠️ O documento da CONSULTA fica FORA do pool: ele contém a própria equação, e
recuperá-lo seria acerto trivial.
"""
...
from avaliar_ablacao_secundarias import bootstrap_pareado_itens  # noqa: E402
...
def montar_pool(itens: pl.DataFrame, todos: np.ndarray, n_pool: int, semente: int,
    ...
    """Alvos de todos os itens + documentos sorteados, SEM os documentos de consulta."""
    alvos = {d for lista in itens["alvos"].to_list() for d in lista}
    consultas = set(itens["documento_da_consulta"].to_list())
    if alvos & consultas:
        raise SystemExit("um documento de consulta está entre os alvos")
    fora = alvos | consultas
    candidatos = np.array([d for d in todos if d not in fora], dtype=object)
...
def avaliar(escores: np.ndarray, alvos_idx: list[set[int]]) -> dict:
...
def main() -> int:
    ...
    print(f"itens {len(consultas):,} · pool {len(pool):,} documentos · "
          f"{len(textos):,} equações indexadas", flush=True)

    sistemas, tempos = {}, {}
    t0 = time.perf_counter()
    sistemas["bm25"] = avaliar(pontuar_bm25(consultas, textos, doc_de_cada, len(pool)),
                               alvos_idx)
    tempos["bm25"] = round(time.perf_counter() - t0, 1)
    print(f"  bm25 · recall@10 {sistemas['bm25']['recall_10'].mean():.4f} · "
          f"{tempos['bm25']:.0f} s", flush=True)
    for caminho in a.modelos:
        nome = Path(caminho).name
        t0 = time.perf_counter()
        sistemas[nome] = avaliar(
            pontuar_denso(caminho, consultas, textos, doc_de_cada, len(pool),
                          a.dispositivo, a.max_tokens, a.lote), alvos_idx)
        tempos[nome] = round(time.perf_counter() - t0, 1)
        print(f"  {nome} · recall@10 {sistemas[nome]['recall_10'].mean():.4f} · "
              f"{tempos[nome]:.0f} s", flush=True)

    def resumo(m: dict) -> dict:
        return {**{f"recall_{k}": round(float(m[f'recall_{k}'].mean()), 4) for k in KS},
    ...
    melhor = max(contra_bm25, key=lambda n: contra_bm25[n]["diferenca"])
    desfecho, leitura = ler_pela_regra(contra_bm25[melhor])
